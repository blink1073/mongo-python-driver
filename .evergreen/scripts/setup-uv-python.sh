...
function _ensure_python() {
  local request="$1"
  if uv python find ${request:+"$request"} > /dev/null 2>&1; then
    return 0
  fi
  echo "Python \"$request\" was not found on this host; asking uv to install it..."
  if uv python install ${request:+"$request"}; then
    return 0
  fi
...
# Point uv at the toolchain Python before pinning uv, so `uv tool install` uses
# it instead of downloading a managed interpreter.
